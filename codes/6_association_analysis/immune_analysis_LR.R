...
immune <- read.table("data/1_sampleSheet/hh_ova_immune.txt")
tmb <- read.csv("/Volumes/Wild_Flower/OV_CopyNumber/docs/00_mitéra/clinical/raw/ova/HH_TMB.csv",
...
gencode_counts <- read.csv("data/3_flair/gencode.v39/hugo_tpm.csv", row.names = 1, check.names = F)
...
combined_immune <- merge(combined_immune, metatable[, c("copy_number", "long_read", "group")], by.x = "Row.names", by.y = "copy_number")
combined_immune <- merge(combined_immune, mcp_results, by.x="long_read", by.y="row.names")
combined_immune <- merge(combined_immune, tls_rna_seq[, c("Row.names", "TLS_mean")], by.x = "long_read", by.y="Row.names")
combined_immune <- merge(combined_immune, immune_genes, by.x="long_read", by.y="row.names")
rownames(combined_immune) <- combined_immune$Row.names
combined_immune$Row.names <- NULL
combined_immune <- combined_immune[colnames(gencode_counts), ] ## align 
...
save.image("data/6_association/sample_data/immune_mtx.Rd")
