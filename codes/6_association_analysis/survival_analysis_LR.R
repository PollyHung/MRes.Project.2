...
## average cluster PSI
PSI <- read.table("data/5_differential_expression/SUPPA2/diffSplice_gc.psivec")
colnames(PSI) <- c(colnames(psi_tumour), colnames(psi_normal))

PSI_cluster_avg <- data.frame(
  sample_id = colnames(PSI), 
  cluster_0 = colMeans(PSI[pull(filter(cluster_gc_filt, cluster_id == 0), "event_id"), ]), 
  cluster_1 = colMeans(PSI[pull(filter(cluster_gc_filt, cluster_id == 1), "event_id"), ]), 
  cluster_2 = colMeans(PSI[pull(filter(cluster_gc_filt, cluster_id == 2), "event_id"), ]), 
  cluster_3 = colMeans(PSI[pull(filter(cluster_gc_filt, cluster_id == 3), "event_id"), ]), 
  cluster_4 = colMeans(PSI[pull(filter(cluster_gc_filt, cluster_id == 4), "event_id"), ]), 
  cluster_5 = colMeans(PSI[pull(filter(cluster_gc_filt, cluster_id == 5), "event_id"), ]), 
  cluster_6 = colMeans(PSI[pull(filter(cluster_gc_filt, cluster_id == 6), "event_id"), ]), 
  cluster_7 = colMeans(PSI[pull(filter(cluster_gc_filt, cluster_id == 7), "event_id"), ]))

write.table(PSI_cluster_avg, "data/5_differential_expression/SUPPA2/PSI_cluster_avg.txt", 
            row.names = F, col.names = T, sep = "\t", quote = F)

## mapping table 
mapping <- read.csv("data/0_samples/mapping.csv") %>% 
  dplyr::mutate(Sample = gsub("_| ", "-", Sample)) %>% 
  dplyr::select(Sample, ICBRC, X.1) %>% 
  dplyr::rename(long_read = Sample, copy_number = ICBRC, short_read = X.1) %>% 
  dplyr::filter(long_read %in% PSI_cluster_avg$sample_id)

## sample table 
survival <- read.table("data/0_samples/hh_ova_surv.txt", row.names = NULL) %>% 
  dplyr::filter(row.names %in% mapping$copy_number) %>% 
  dplyr::rename(copy_number = row.names)
immune <- read.table("data/0_samples/hh_ova_immune.txt", row.names = NULL) %>% 
  dplyr::filter(row.names %in% mapping$copy_number) %>% 
  dplyr::rename(copy_number = row.names)
clinical <- read.table("data/0_samples/hh_ova.txt", row.names = NULL) %>% 
  dplyr::filter(row.names %in% mapping$copy_number) %>% 
  dplyr::rename(copy_number = row.names)

## metatable 
metatable <- merge(PSI_cluster_avg, mapping, by.x = "sample_id", by.y = "long_read")
metatable <- merge(metatable, survival, by = "copy_number")
metatable <- merge(metatable, immune, by = "copy_number")
metatable <- merge(metatable, clinical, by = "copy_number")

write.table(PSI_cluster_avg, "data/5_differential_expression/SUPPA2/metatable.txt", 
            row.names = F, col.names = T, sep = "\t", quote = F)


library(survival)
## overall survival 
os <- matrix(NA, nrow = length(unique(cluster_gc_filt$cluster_id)), ncol = 4) %>% as.data.frame()
colnames(os) <- c("HR", "upper_95", "lower_95", "p_val")
rownames(os) <- paste0("cluster_", 0:7)
surv_obj <- Surv(metatable$os_time, metatable$os_event)

for(cluster in rownames(os)){
  tryCatch({
    coxph_model <- coxph(surv_obj ~ metatable[, cluster] + age + stage, data = metatable)
    model <- summary(coxph_model) 
    os[cluster, "HR"] <- model$coefficients[1, "coef"]
    os[cluster, "p_val"] <- model$coefficients[1, "Pr(>|z|)"]
    os[cluster, "upper_95"] <- model$conf.int[1, "upper .95"]
    os[cluster, "lower_95"] <- model$conf.int[1, "lower .95"]
  }, error = function(e) {
    cat("Error with isoform", cluster, ":", conditionMessage(e), "\n")
  })
}
os$p_adj <- p.adjust(os$p_val, method = "fdr")
...
# Define the coordinates for the splicing event
exons <- GRanges(
  seqnames = Rle(chromosome),
  ranges = IRanges(
    start = c(44531978, 44582731),
    end = c(44582585, 44589797)
  ),
  strand = Rle(strand)
)

# Add exon identifiers
exons$exon_id <- c("Exon1", "Exon2")

# Create a transcript track
txTrack <- AnnotationTrack(
  exons,
  genome = "hg38",
  name = "Alternative Splicing",
  feature = exons$exon_id
)

# Create a genome axis track
genomeAxis <- GenomeAxisTrack()

# Create a gene region track using TxDb
...
plotTracks(
  list(genomeAxis, geneTrack, txTrack),
  from = 44531970,
  to = 44589800,
  chromosome = chromosome,
  main = "Alternative Splicing Event for Gene PKIG (PB.14043)"
)
