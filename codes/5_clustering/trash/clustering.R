...
transdecoder <- read.csv("data/4_transcoder/transdecoder_map.csv", row.names = 1) %>% 
  dplyr::mutate(PB_ID = sub("\\.[^.]*$", "", PBid)) %>% 
  dplyr::select("PB_ID", "Gencode_gene_name") %>% 
  distinct()
classification <- read.table("data/2_sqanti/filter_rule_RulesFilter_result_classification.txt", header = TRUE) %>% 
  dplyr::mutate(PB_ID = sub("\\.[^.]*$", "", isoform)) %>% 
  dplyr::select("PB_ID", "associated_gene") %>% 
  dplyr::filter(PB_ID %in% transdecoder$PB_ID[which(is.na(transdecoder$Gencode_gene_name))]) %>% 
  group_by(PB_ID) %>%
  filter(nchar(associated_gene) == max(nchar(associated_gene))) %>%
  ungroup() %>% 
  as.data.frame() %>% 
  dplyr::mutate(associated_gene = sub("(novelGene_\\d+).*", "novelGene", associated_gene)) %>% 
  distinct() 
transdecoder <- left_join(transdecoder, classification, by = "PB_ID")
transdecoder <- transdecoder %>% 
  dplyr::mutate(Gencode_gene_name = coalesce(Gencode_gene_name, associated_gene)) %>% 
  dplyr::select("PB_ID", "Gencode_gene_name")
rownames(psi_isoforms) <- sub(".*?;", "", rownames(psi_isoforms))
psi_events$rownames <- rownames(psi_events)
psi_events <- psi_events %>% 
  separate(rownames, into = c("PB_ID", "details"), sep = ";") %>% 
  separate(details, into = c("events", "rest"), sep = ":", extra = "merge") %>% 
  dplyr::mutate(strand = sub(".*:", "", rest)) %>% 
  dplyr::select("PB_ID", "events", "strand", all_of(samples))
psi_events <- merge(psi_events, transdecoder, by="PB_ID")
rownames(psi_events) <- paste(psi_events$Gencode_gene_name, psi_events$events, psi_events$strand, sep = ":")
